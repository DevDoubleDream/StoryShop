...
import kr.wdream.ui.ProfileActivity;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void blockUser(int user_id) {
        Log.d("MessagesController", "blockUser");
    ...
    public void unblockUser(int user_id) {
        ...
        req.id = getInputUser(user);
        Log.d("MessagesController", "blockedUser : " + NotificationCenter.blockedUsersDidLoaded);
        NotificationCenter.getInstance().postNotificationName(NotificationCenter.blockedUsersDidLoaded);
        ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
            @Override
            public void run(TLObject response, TLRPC.TL_error error) {
    ...
    public void loadDialogs(final int offset, final int count, boolean fromCache) {
        ...
        Log.d("MessagesController", "load cacheOffset : " + offset + " count = " + count + " cache = " + fromCache);

        FileLog.e("tmessages", "load cacheOffset = " + offset + " count = " + count + " cache = " + fromCache);
        if (fromCache) {
            //fromCache가 어떤 역할인지 아직 모르겠음
            MessagesStorage.getInstance().getDialogs(offset == 0 ? 0 : nextDialogsCacheOffset, count);
            Log.d("MessagesController", "fromCache getDialog : " +  nextDialogsCacheOffset);
        } else {
            Log.d("MessagesController", "fromCache false");
            TLRPC.TL_messages_getDialogs req = new TLRPC.TL_messages_getDialogs();
            Log.d("MessagesController", "req : " + req);
            req.limit = count;
            boolean found = false;

            //채팅방 : dialog
            Log.d("MessagesController", "dialog.size : " + dialogs.size());
            for (int a = dialogs.size() - 1; a >= 0; a--) {
                TLRPC.TL_dialog dialog = dialogs.get(a);

                Log.d("MessagesController", "dialog.get(a) : " + dialogs.get(a));

                int lower_id = (int) dialog.id;
                int high_id = (int) (dialog.id >> 32);
                if (lower_id != 0 && high_id != 1 && dialog.top_message > 0) {
                    MessageObject message = dialogMessage.get(dialog.id);
                    Log.d("MessagesController", "message : " + message.getId());
                    if (message != null && message.getId() > 0) {
                        Log.d("MessagesController", "message.messageOwner.date : " + message.messageOwner.date);
                        Log.d("MessagesController", "message.messageOwner.to_id.channel_id : " + message.messageOwner.to_id.channel_id);
                        Log.d("MessagesController", "message.messageOwner.to_id.chat_id : " + message.messageOwner.to_id.chat_id);
                        ...
                        if (message.messageOwner.to_id.channel_id != 0) {
                            ...
                            Log.d("MessagesController", "message channel : " + message.messageOwner.to_id.channel_id);
                        } else if (message.messageOwner.to_id.chat_id != 0) {
                            Log.d("MessagesController", "message chat_id : " + message.messageOwner.to_id.chat_id);
                            id = -message.messageOwner.to_id.chat_id;
                        } else {
                            Log.d("MessagesController", "message user_id : " + message.messageOwner.to_id.user_id);
    ...
    public static void openByUserName(String username, final BaseFragment fragment, final int type) {
        Log.d("MessagesController", "openByUserName Start");

        if (username == null || fragment == null) {
            Log.d("MessagesController", "username fragment null");
            return;
        }
        TLRPC.User user = getInstance().getUser(username);
        if (user != null) {
            Log.d("MessagesController", "openByUserName(type) : " + type);
