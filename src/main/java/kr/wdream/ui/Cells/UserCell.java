...
import kr.wdream.ui.Components.CheckBoxSquare;
...
public class UserCell extends FrameLayout {
    ...
    public void setIsAdmin(int value) {
        ...
        if (value == 1) {
            adminImage.setImageResource(kr.wdream.storyshop.R.drawable.admin_star);
        } else if (value == 2) {
            adminImage.setImageResource(kr.wdream.storyshop.R.drawable.admin_star2);
        }
    }
        ...
        if (currrntStatus != null) {
            ...
            if (currentUser.bot) {
                ...
                if (currentUser.bot_chat_history || adminImage != null && adminImage.getVisibility() == VISIBLE) { //TODO fix
                    statusTextView.setText(LocaleController.getString("BotStatusRead", kr.wdream.storyshop.R.string.BotStatusRead));
                } else {
                    statusTextView.setText(LocaleController.getString("BotStatusCantRead", kr.wdream.storyshop.R.string.BotStatusCantRead));
                }
            } else {
                if (currentUser.id == UserConfig.getClientUserId() || currentUser.status != null && currentUser.status.expires > ConnectionsManager.getInstance().getCurrentTime() || MessagesController.getInstance().onlinePrivacy.containsKey(currentUser.id)) {
                    ...
                    statusTextView.setText(LocaleController.getString("Online", kr.wdream.storyshop.R.string.Online));
                } else {
                    statusTextView.setTextColor(statusColor);
