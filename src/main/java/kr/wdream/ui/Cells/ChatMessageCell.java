...
import kr.wdream.ui.Components.LinkPath;
import kr.wdream.ui.Components.RadialProgress;
import kr.wdream.ui.Components.SeekBar;
import kr.wdream.ui.Components.SeekBarWaveform;
import kr.wdream.ui.Components.StaticLayoutEx;
import kr.wdream.ui.ActionBar.Theme;
import kr.wdream.ui.Components.TypefaceSpan;
import kr.wdream.ui.Components.URLSpanBotCommand;
import kr.wdream.ui.Components.URLSpanNoUnderline;
import kr.wdream.ui.PhotoViewer;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
    ...
    public ChatMessageCell(Context context) {
        ...
        gamePaint.setTextSize(AndroidUtilities.dp(13));
    ...
    private int createDocumentLayout(int maxWidth, MessageObject messageObject) {
        ...
        if (MessageObject.isVoiceDocument(documentAttach)) {
            ...
            if (name == null || name.length() == 0) {
                name = LocaleController.getString("AttachDocument", kr.wdream.storyshop.R.string.AttachDocument);
            }
        ...
        if (messageChanged || dataChanged || isPhotoDataChanged(messageObject)) {
            ...
            if (messageObject.type == 0) {
                    ...
                    if (title != null) {
                        try {
                            ...
                            if (!isSmallImage || description == null) {
                                titleLayout = StaticLayoutEx.createStaticLayout(title, replyNamePaint, linkPreviewMaxWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, AndroidUtilities.dp(1), false, TextUtils.TruncateAt.END, linkPreviewMaxWidth, 4);
                            } else {
                                restLines = restLinesCount;
                                titleLayout = generateStaticLayout(title, replyNamePaint, linkPreviewMaxWidth, linkPreviewMaxWidth - AndroidUtilities.dp(48 + 4), restLinesCount, 4);
                    ...
                    if (author != null) {
                        try {
                            ...
                            if (restLinesCount == 3 && (!isSmallImage || description == null)) {
                                ...
                                authorLayout = generateStaticLayout(author, replyNamePaint, linkPreviewMaxWidth, linkPreviewMaxWidth - AndroidUtilities.dp(48 + 4), restLinesCount, 1);
                    ...
                    if (documentAttachType != DOCUMENT_ATTACH_TYPE_MUSIC && documentAttachType != DOCUMENT_ATTACH_TYPE_AUDIO && documentAttachType != DOCUMENT_ATTACH_TYPE_DOCUMENT) {
                        if (currentPhotoObject != null) {
                            ...
                            if (type != null && type.equals("video") && duration != 0) {
                                ...
                                String str = LocaleController.getString("AttachGame", kr.wdream.storyshop.R.string.AttachGame).toUpperCase();
                ...
                if (phone != null && phone.length() != 0) {
                    ...
                    phone = LocaleController.getString("NumberUnknown", kr.wdream.storyshop.R.string.NumberUnknown);
                }
    ...
    private void drawContent(Canvas canvas) {
                ...
                if (drawPhotoImage) {
                    ...
                    if (videoInfoLayout != null) {
                        ...
                        if (hasGamePreview) {
                            ...
                            Theme.timeBackgroundDrawable.setBounds(x - AndroidUtilities.dp(4), y - AndroidUtilities.dp(1.5f), x + durationWidth + AndroidUtilities.dp(4), y + AndroidUtilities.dp(16.5f));
        ...
        if (captionLayout != null) {
            ...
            if (currentMessageObject.type == 1 || documentAttachType == DOCUMENT_ATTACH_TYPE_VIDEO || currentMessageObject.type == 8) {
                ...
                canvas.translate(captionX = backgroundDrawableLeft + AndroidUtilities.dp(currentMessageObject.isOutOwner() ? 11 : 17), captionY = totalHeight - captionHeight - AndroidUtilities.dp(10));
            }
            if (pressedLink != null) {
                for (int b = 0; b < urlPath.size(); b++) {
                    canvas.drawPath(urlPath.get(b), urlPaint);
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if (messageObject.messageOwner.via_bot_id == 0 && messageObject.messageOwner.via_bot_name == null && (author == null || !author.bot) && (messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_EDITED) != 0) {
            timeString = LocaleController.getString("EditedMessage", kr.wdream.storyshop.R.string.EditedMessage) + " " + LocaleController.getInstance().formatterDay.format((long) (messageObject.messageOwner.date) * 1000);
        } else {
    ...
    private void setMessageObjectInternal(MessageObject messageObject) {
        ...
        if (drawForwardedName && messageObject.isForwarded()) {
            ...
            if (currentForwardUser != null || currentForwardChannel != null) {
                ...
                int fromWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", kr.wdream.storyshop.R.string.From) + " "));
                ...
                if (viaString != null) {
                    viaNameWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", kr.wdream.storyshop.R.string.From) + " " + name));
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b> via <b>%s</b>", LocaleController.getString("From", kr.wdream.storyshop.R.string.From), name, viaUsername));
                } else {
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b>", LocaleController.getString("From", kr.wdream.storyshop.R.string.From), name));
                }
                ...
                try {
                    ...
                    lastLine = TextUtils.ellipsize(AndroidUtilities.replaceTags(LocaleController.getString("ForwardedMessage", kr.wdream.storyshop.R.string.ForwardedMessage)), forwardNamePaint, forwardedNameWidth, TextUtils.TruncateAt.END);
        ...
        if (messageObject.isReply()) {
            ...
            if (messageObject.replyMessageObject != null) {
                ...
                if (messageObject.replyMessageObject.messageOwner.media instanceof TLRPC.TL_messageMediaGame) {
                    stringFinalText = Emoji.replaceEmoji(messageObject.replyMessageObject.messageOwner.media.game.title, replyTextPaint.getFontMetricsInt(), AndroidUtilities.dp(14), false);
            ...
            if (stringFinalName == null) {
                stringFinalName = LocaleController.getString("Loading", kr.wdream.storyshop.R.string.Loading);
            }
            try {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentMessageObject.isOutOwner()) {
            ...
            setDrawableBounds(currentBackgroundDrawable, backgroundDrawableLeft = layoutWidth - backgroundWidth - (!mediaBackground ? 0 : AndroidUtilities.dp(9)), AndroidUtilities.dp(1), backgroundWidth - (mediaBackground ? 0 : AndroidUtilities.dp(3)), layoutHeight - AndroidUtilities.dp(2));
        } else {
            if (isDrawSelectedBackground()) {
                if (!mediaBackground) {
            ...
            if (isChat && currentMessageObject.isFromUser()) {
                setDrawableBounds(currentBackgroundDrawable, backgroundDrawableLeft = AndroidUtilities.dp(48 + (!mediaBackground ? 3 : 9)), AndroidUtilities.dp(1), backgroundWidth - (mediaBackground ? 0 : AndroidUtilities.dp(3)), layoutHeight - AndroidUtilities.dp(2));
            } else {
                setDrawableBounds(currentBackgroundDrawable, backgroundDrawableLeft = (!mediaBackground ? AndroidUtilities.dp(3) : AndroidUtilities.dp(9)), AndroidUtilities.dp(1), backgroundWidth - (mediaBackground ? 0 : AndroidUtilities.dp(3)), layoutHeight - AndroidUtilities.dp(2));
            }
        }
