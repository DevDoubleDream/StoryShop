...
public class StickerMasksView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    private void updateStickerTabs() {
        ...
        if (currentType == StickersQuery.TYPE_IMAGE) {
            scrollSlidingTabStrip.addIconTab(kr.wdream.storyshop.R.drawable.ic_masks_msk1);
            stickersEmptyView.setText(LocaleController.getString("NoStickers", kr.wdream.storyshop.R.string.NoStickers));
        } else {
            scrollSlidingTabStrip.addIconTab(kr.wdream.storyshop.R.drawable.ic_masks_sticker1);
            stickersEmptyView.setText(LocaleController.getString("NoMasks", kr.wdream.storyshop.R.string.NoMasks));
        }

        if (!recentStickers[currentType].isEmpty()) {
            recentTabBum = stickersTabOffset;
            stickersTabOffset++;
            scrollSlidingTabStrip.addIconTab(kr.wdream.storyshop.R.drawable.ic_masks_recent);
        }
